...
// Старый вебхук для Stars платежей (для совместимости)
router.post('/stars/webhook', async (req, res) => {
  try {
    ...
    if (update.message && update.message.successful_payment) {
      ...
      console.log('✅ [STARS-WEBHOOK] Платеж обработан:', invoice.order_id);
    }
    
    res.json({ ok: true });
  } catch (error) {
    console.error('❌ [STARS-WEBHOOK] Ошибка:', error);
    res.status(500).json({ error: 'Ошибка обработки Stars вебхука' });
  }
});
